...
namespace MainOffice.Controllers
{
    ...
    public class BillsController : Controller
    {
        ...
        public ActionResult Index(int? Edit)
        {
            ...
            var bills = db.Bills.Include(b => b.Client).Include(b => b.Employee).Include(b => b.Salon);
        ...
        public async Task<ActionResult> Create([Bind(Include = "Id,BillNumber,IssueDate,SalonId,EmployeeId,StartTime,EndTime,BO,ClientId,BillLines")] Bill bill)
        {
            if (ModelState.IsValid)
            {
                ...
                if (employeeId.HasValue)
                {
                    ...
                    {
                        bill = await db.Bills.Include(b => b.Client).Include(b => b.Employee).Include(b => b.Salon).FirstAsync(b => b.Id == bill.Id);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "Id,BillNumber,IssueDate,SalonId,EmployeeId,StartTime,EndTime,BO,ClientId,RowVersion,BillLines")] Bill bill)
        {
            ViewBag.Concurrency = false;
            if (ModelState.IsValid)
            {
        ...
        {
            Bill bill = await db.Bills.Include(b => b.Client).Include(b => b.Employee).Include(b => b.Salon).SingleOrDefaultAsync(b => b.Id == id);
            if (bill == null)
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Filter([Bind(Include = "IssueDateFrom,IssueDateTo,SalonIdSelected,EmployeeIdSelected,StartTimeFrom,StartTimeTo,EndTimeFrom,EndTimeTo,BO,PrintCountFrom,PrintCountTo")] BillFilter dataFilter)
        {
            if (ModelState.IsValid)
            {
        ...
        private IQueryable<Bill> BuildFilter(IQueryable<Bill> bills, BillFilter filter)
        {
            ...
            if (filter.PrintCountFrom != null)
                bills = bills.Where(b => b.PrintCount >= filter.PrintCountFrom);
            if (filter.PrintCountTo != null)
                bills = bills.Where(b => b.PrintCount <= filter.PrintCountTo);
